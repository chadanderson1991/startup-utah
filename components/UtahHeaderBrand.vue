<script setup lang="ts">
// Mirrors the brand row at the top of startup.utah.gov:
// UTAH wordmark + "An official website" | Startup State logo | GOEO logo
// Separated by thin vertical rules.
</script>

<template>
  <div class="utah-header__top flex items-center gap-0">
    <!-- UTAH wordmark with "An official website" -->
    <a
      href="https://utah.gov/"
      target="_blank"
      rel="noopener noreferrer"
      class="flex items-center pr-5 py-2 shrink-0"
      aria-label="Utah.gov — an official website of the State of Utah"
    >
      <svg
        class="utah-header__utah-brand h-12 w-auto"
        viewBox="0 0 105.9496 47.6226"
        aria-hidden="true"
      >
        <g>
          <g>
            <path d="M12.2714,30.0877c-4.1489,0-7.2318-1.2037-9.2489-3.611C1.0055,24.0693-.002,20.334,0,15.2709V0H7.8175V16.1806c0,2.6363,.356,4.4923,1.0679,5.5679,.7613,1.1018,2.0506,1.7162,3.3859,1.6134,1.3465,.0953,2.6458-.5157,3.4313-1.6134,.7422-1.0756,1.1133-2.9316,1.1133-5.5679V0h7.5448V15.2709c0,5.0601-.9847,8.7946-2.9541,11.2035-1.9694,2.4089-5.0145,3.6133-9.1352,3.6133Zm24.0887-.5463V6.5444h-7.8175V0h23.4526V6.5444h-7.8175V29.5414h-7.8175Zm25.8151-14.362l-.5002,2.0452h5.455l-.5002-2.0452c-.3637-1.4239-.7273-2.9693-1.091-4.636-.3637-1.6667-.7261-3.242-1.0871-4.7259h-.1821c-.3334,1.5151-.6743,3.0983-1.0226,4.7497s-.7053,3.189-1.071,4.6129l-.0008-.0008Zm-11.3617,14.362L59.8127,0h9.4502l9.0023,29.5414h-8.2724l-1.4544-6.2709h-8.2716l-1.4544,6.2709h-7.9988Zm30.2713,0V0h7.8175V10.9991h8.8171V0h7.8175V29.5414h-7.8175v-11.7251h-8.8194v11.7251h-7.8152Z" />
          </g>
          <text
            transform="translate(.0419 43.5205)"
            style="font-family: 'Source Sans 3', 'Source Sans Pro'; font-size: 14px;"
          >An official website</text>
        </g>
      </svg>
    </a>

    <span class="utah-header__vertical-rule self-stretch" />

    <!-- Startup State logo -->
    <NuxtLink
      to="/"
      class="flex items-center px-5 py-2 shrink-0"
      aria-label="Startup State Initiative home"
    >
      <img
        src="/brand/StartupState_Logo_Web_Color_Horiz.jpg"
        alt="Startup State Initiative · Utah"
        class="h-12 w-auto"
      />
    </NuxtLink>

    <span class="utah-header__vertical-rule self-stretch" />

    <!-- Governor's Office of Economic Opportunity logo -->
    <a
      href="https://business.utah.gov/"
      target="_blank"
      rel="noopener noreferrer"
      class="flex items-center pl-5 py-2 shrink-0"
      aria-label="Governor's Office of Economic Opportunity"
    >
      <img
        src="/brand/GOEO_BLACK_Standard_Web.webp"
        alt="Governor's Office of Economic Opportunity"
        class="h-10 w-auto"
      />
    </a>
  </div>
</template>

<style scoped>
.utah-header__utah-brand {
  fill: #707070;
}
.utah-header__vertical-rule {
  display: inline-block;
  width: 1px;
  background-color: #d7d7d7;
  align-self: stretch;
}
</style>
